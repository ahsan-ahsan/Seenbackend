...
exports.checkRcoinToDiamondConversion = async (req, res) => {
  try {
    const { rCoin } = req.query;

    if (!rCoin || isNaN(rCoin)) {
      return res.status(400).json({
        status: false,
        message: "Invalid or missing rCoin value",
      });
    }

    const setting = await Setting.findOne({});
    if (!setting || !setting.rCoinForDiamond) {
      return res.status(500).json({
        status: false,
    ...
    const rCoinValue = Number(rCoin);
    const diamondsToAdd = Math.floor(rCoinValue / setting.rCoinForDiamond);

    return res.status(200).json({
      status: true,
      message: "Conversion calculated successfully",
      inputRCoin: rCoinValue,
      rCoinPerDiamond: setting.rCoinForDiamond,
